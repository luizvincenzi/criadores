...
${posts.map(post => {
  ...
  const cleanContent = cleanHtml(post.content).substring(0, 500) + '...';
  const cleanExcerpt = cleanHtml(post.excerpt);

  // Mapear audience_target para categoria legível
  const categoryMap = {
  ...
  return `    <item>
      ...
      <category><![CDATA[${categoryMap[post.audience_target] || 'Geral'}]]></category>
