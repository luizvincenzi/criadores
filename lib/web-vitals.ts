...
export function reportWebVitals(metric: Metric) {
  ...
  try {
    ...
    // Reportar para endpoint próprio (opcional - para análise detalhada)
    if (typeof window !== 'undefined' && window.fetch) {
      ...
      fetch('/api/web-vitals', {
        ...
        console.warn('⚠️ [WEB VITALS] Erro ao enviar para endpoint:', error);
      });
    }
...
/**
 * Configuração para web-vitals library
 */
export const WEB_VITALS_CONFIG = {
  // Reportar apenas quando a página está sendo "unloaded" para dados mais precisos
  reportAllChanges: false,
  
  // Threshold para reportar mudanças (apenas mudanças significativas)
  threshold: 0.001,
} as const;
