-- =====================================================
-- MIGRATION 030: Adicionar Controle de Acesso à Plataforma
-- =====================================================
...
-- 3.1. Trigger para sincronizar CREATORS → PLATFORM_USERS
...
-- 3.2. Trigger para sincronizar BUSINESSES → PLATFORM_USERS
...
-- =====================================================
-- PARTE 6: ROW LEVEL SECURITY (RLS) - LGPD COMPLIANCE
-- =====================================================

-- 6.1. Habilitar RLS nas tabelas críticas
ALTER TABLE platform_users ENABLE ROW LEVEL SECURITY;
ALTER TABLE businesses ENABLE ROW LEVEL SECURITY;
ALTER TABLE creators ENABLE ROW LEVEL SECURITY;
ALTER TABLE campaigns ENABLE ROW LEVEL SECURITY;

-- 6.2. Política para BUSINESS_OWNER - Acesso APENAS ao seu business
CREATE POLICY "business_owner_access_own_business" ON businesses
  FOR SELECT
  TO authenticated
  USING (
    -- Usuário é business_owner E o business está em managed_businesses
    EXISTS (
    ...
    OR
    -- OU é funcionário interno (CRM)
    EXISTS (
      SELECT 1 FROM users u
      WHERE u.id = auth.uid()
      AND u.role IN ('admin', 'manager', 'user')
    )
  );

-- 6.3. Política para CAMPAIGNS - Business owner vê APENAS suas campanhas
CREATE POLICY "business_owner_access_own_campaigns" ON campaigns
  FOR SELECT
  TO authenticated
  USING (
    -- Usuário é business_owner E a campanha é do seu business
    EXISTS (
      SELECT 1 FROM platform_users pu
      WHERE pu.id = auth.uid()
      AND 'business_owner' = ANY(pu.roles)
    ...
    OR
    -- OU é creator da campanha
    EXISTS (
      SELECT 1 FROM platform_users pu
      WHERE pu.id = auth.uid()
      AND 'creator' = ANY(pu.roles)
      AND campaigns.id IN (
        SELECT campaign_id FROM campaign_creators
        WHERE creator_id = pu.creator_id
      )
    )
    OR
    -- OU é marketing_strategist com acesso ao business
    EXISTS (
      SELECT 1 FROM platform_users pu
      WHERE pu.id = auth.uid()
      AND 'marketing_strategist' = ANY(pu.roles)
      AND campaigns.business_id = ANY(pu.managed_businesses)
    )
    OR
    -- OU é funcionário interno (CRM)
    EXISTS (
...
-- 6.4. Política para CREATORS - Creator vê apenas seus dados
CREATE POLICY "creator_access_own_data" ON creators
  FOR SELECT
  TO authenticated
  USING (
    -- Usuário é o próprio creator
    EXISTS (
      SELECT 1 FROM platform_users pu
      WHERE pu.id = auth.uid()
      AND pu.creator_id = creators.id
    )
    OR
    -- OU é marketing_strategist que trabalha com este creator
    EXISTS (
      SELECT 1 FROM platform_users pu
      WHERE pu.id = auth.uid()
      AND 'marketing_strategist' = ANY(pu.roles)
...
-- 6.5. Política para PLATFORM_USERS - Usuário vê apenas seus dados
...
-- 6.6. Função auxiliar para verificar acesso a business
...
-- 6.7. Função auxiliar para listar businesses do usuário
...
-- 6.8. Constraint para garantir que business_owner tem business_id
...
-- 6.9. Constraint para garantir que creator tem creator_id
ALTER TABLE platform_users
...
CREATE INDEX idx_access_audit_user ON platform_access_audit(user_id, created_at DESC);
CREATE INDEX idx_access_audit_resource ON platform_access_audit(resource_type, resource_id);
...
SELECT 'RLS habilitado e políticas de segurança criadas!' as security_status;
SELECT 'LGPD Compliance: Isolamento total por business garantido!' as lgpd_status;
