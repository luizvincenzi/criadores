...
export async function getRawCampaignsData(): Promise<CampaignData[]> {
  try {
    ...
    const rawCampaigns: CampaignData[] = values.slice(1)
      ...
      .map((row, index) => {
        if (hasIdColumn) {
          // Nova estrutura com ID - MAPEAMENTO CORRIGIDO
          return {
            id: row[0] || `raw-campaign-${index + 1}`,
            campaignId: row[0] || '',
            campanha: row[1] || '',
            business: row[1] || '',        // B = Business_ID (corrigido)
            influenciador: row[2] || '',   // C = Creator_ID (corrigido)
            responsavel: row[3] || '',     // D = Responsável
            status: row[4] || 'Ativa',     // E = Status (corrigido)
            mes: row[5] || '',             // F = Mês (corrigido)
            fim: row[6] || '',             // G = FIM (corrigido)
            briefingCompleto: row[7] || '', // H = Briefing (corrigido)
            dataHoraVisita: row[8] || '',   // I = Data Visita (corrigido)
            quantidadeConvidados: row[9] || '', // J = Quantidade (corrigido)
            visitaConfirmado: row[10] || '',    // K = Visita Confirmado (corrigido)
            dataHoraPostagem: row[11] || '',    // L = Data Postagem (corrigido)
            videoAprovado: row[12] || '',       // M = Video Aprovado (corrigido)
            videoPostado: row[13] || '',        // N = Video Postado (corrigido)
