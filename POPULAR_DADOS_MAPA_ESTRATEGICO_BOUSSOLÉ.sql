...
-- Business ID será obtido dinamicamente baseado no usuário financeiro.brooftop@gmail.com

-- 1. Primeiro, vamos obter ou criar o business_id da Boussolé
-- Assumindo que já existe um business com esse nome, vamos buscar o ID
-- Se não existir, você pode criar manualmente ou ajustar conforme necessário

-- 2. Inserir dados REALISTAS na tabela strategic_maps
INSERT INTO strategic_maps (
  ...
  (SELECT id FROM organizations LIMIT 1), -- Usar primeira organização disponível
  (SELECT id FROM businesses WHERE name ILIKE '%boussolé%' LIMIT 1), -- Buscar business Boussolé
  '2025-Q4',
  2025,
  4,
  'completed',
...
-- 3. Inserir seções do mapa estratégico
-- Primeiro, obter o ID do mapa estratégico que acabamos de criar
...
INSERT INTO strategic_map_sections (
  ...
  (SELECT id FROM strategic_map_id),
  'metrics_overview',
  1,
  '{
    "instagram": {
...
-- Seção 6: ICP (Ideal Customer Profile)
INSERT INTO strategic_map_sections (
  strategic_map_id,
  section_type,
  section_order,
...
INSERT INTO strategic_map_sections (
  ...
  '{
    "strategic_alignment": "Os objetivos estão alinhados com a análise SWOT e o diagnóstico do negócio, focando nas oportunidades identificadas.",
